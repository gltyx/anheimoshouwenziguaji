...
function HandleStartServer(eventType, eventData)
    -- Must use TRANSPORT_SLIKENET (RakNet/SLikeNet) for LAN Discovery features
    -- SetDiscoveryBeacon and DiscoverHosts are RakNet-specific
    if network:StartServerWithTransport(SERVER_PORT, TRANSPORT_SLIKENET) == true
    then
