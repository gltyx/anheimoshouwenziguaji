#ifndef CLUSTER_UTIL_SH_HEADER_GUARD
#define CLUSTER_UTIL_SH_HEADER_GUARD
...
//    );
    ...
    vec3 ndc = vec3(
        2.0 * (coord.x - 0.0) / u_viewRect.z - 1.0,
        2.0 * (coord.y - 0.0) / u_viewRect.w - 1.0,
        2.0 * coord.z - 1.0 // -> [-1, 1]
    );
#else
//    vec3 ndc = vec3(
...
//    );
    vec3 ndc = vec3(
        2.0 * (coord.x - 0.0) / u_viewRect.z - 1.0,
        2.0 * (u_viewRect.w - coord.y - 1 - 0.0) / u_viewRect.w - 1.0, // y is flipped
        coord.z // -> [0, 1]
    );
#endif
    ...
    // 注意行列矩阵，sample和我们相反
    vec4 eye = mul(vec4(ndc, 1.0), u_invProj);
    eye = eye / eye.w;
...
hfloat screen2EyeDepth(hfloat depth, hfloat near, hfloat far)
{
    // https://stackoverflow.com/a/45710371/862300

#if BGFX_SHADER_LANGUAGE_GLSL || BGFX_SHADER_LANGUAGE_GLSL_HLSLCC
    hfloat ndc = 2.0 * depth - 1.0;
    // ndc = (eye * (far + near) / (far - near) - 2 * (far * near) / (far - near)) / eye
    hfloat eye = 2.0 * far * near / (far + near + ndc * (near - far));
#else
    hfloat ndc = depth;
    // ndc = (eye * far / (far - near) - (far * near) / (far - near)) / eye
    hfloat eye = far * near / (far + ndc * (near - far));
#endif
