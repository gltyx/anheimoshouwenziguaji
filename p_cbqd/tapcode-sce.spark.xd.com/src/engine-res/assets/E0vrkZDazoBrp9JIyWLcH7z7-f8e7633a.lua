...
function CreateVehicle()
    ...
    vehicleNode.position = Vector3(0.0, 1.5, 0.0)
...
function Vehicle:Init()
    ...
    local hullModelNode = node:CreateChild("VehicleHull")
    hullModelNode.position = Vector3(0, -1.2, 0)
    hullModelNode.rotation = Quaternion(-90, 0, -90)
    hullModelNode.scale = Vector3(0.01 / 4.0, 0.01 / 2.3, 0.01)   
    local hullObject = hullModelNode:CreateComponent("StaticModel")
    hullObject.model = cache:GetResource("Model", "Vehicle/Models/VehicleHull.mdl")
    hullObject.material = cache:GetResource("Material", "Vehicle/Materials/VehicleHull.xml")
	...
	wheelNode.scale = Vector3(1.0, 1.0, 1.0)
    ...
    local wheelModelNode = wheelNode:CreateChild("VehicleWheel")
    wheelModelNode.scale = Vector3(0.0125, 0.0125, 0.0125)
    wheelModelNode.rotation = Quaternion(0, 0, 180)
	local pWheel = wheelModelNode:CreateComponent("StaticModel")
	pWheel.model = cache:GetResource("Model", "Vehicle/Models/VehicleWheel.mdl")
	pWheel.material = cache:GetResource("Material", "Vehicle/Materials/VehicleWheel.xml")
...
function Vehicle:CreateEmitters()
    ...
    for id = 0, raycastVehicle:GetNumWheels() - 1 do
...
function Vehicle:PostUpdate(timeStep)
    ...
    for i = 0, raycastVehicle:GetNumWheels()  - 1 do
	    ...
	    if raycastVehicle:WheelIsGrounded(i) and (raycastVehicle:GetWheelSkidInfoCumulative(i) < 0.9 or raycastVehicle:GetBrake(i) > 2.0 or planeAccel > 15.0) then
            ...
            if particleEmitter.emitting then
		        particleEmitter.emitting = false
	        end
	    end
    end
