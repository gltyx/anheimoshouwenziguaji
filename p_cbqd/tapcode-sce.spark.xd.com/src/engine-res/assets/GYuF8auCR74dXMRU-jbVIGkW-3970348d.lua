...
function HandleSaveNatSettings(eventType, eventData)
    ...
    network:SetNATServerInfo(address, tonumber(port) or 0);
...
function HandleStartServer(eventType, eventData)
    -- Must use TRANSPORT_SLIKENET for NAT Punchtrough (KCP doesn't support NAT)
    network:StartServerWithTransport(SERVER_PORT, TRANSPORT_SLIKENET);
...
function HandleConnect(eventType, eventData)
    ...
    network:AttemptNATPunchtrough(serverGuid.text, nil, userData);
